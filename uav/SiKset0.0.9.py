...
netids = list(range(500))
txpowers = list(range(1,31))
default_serial_port = "/dev/ttyUSB1"

def ser_write(ser, cmd):
...
parser.add_option("-b", "--baud", action="store", type="int", dest="baud", help="Choose our serial connection speed to the radio in baud.  Valid speeds: %s.  If no baud specified, it will test." % (list(serial_speeds.keys())) + " (factory default: 57600)", default="0")
parser.add_option("--serial-speed", action="store", type="int", dest="serial_speed", help="Set the radio's serial speed in baud (SERIAL_SPEED).  Valid speeds: %s." % (list(serial_speeds.keys())) + " (factory default: 57600)", default="0")
...
######
def test_baud():
    for test_baud in reversed(list(serial_speeds.keys())):
...
if options.serial_speed != None:
    if options.serial_speed in list(serial_speeds.keys()):
